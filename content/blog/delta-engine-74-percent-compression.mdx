---
title: "Delta Engine: How Delentia Labs Achieves 74% Memory Compression and Sub-50ms Recall"
...
category: "research"
excerpt: "The Delta Engine is the memory compression and recall system at the core of the RCT Ecosystem. By storing only state changes (deltas) rather than full state snapshots, it targets up to 74% lossless compression and warm recall under 50 milliseconds in internal benchmarks — reducing per-request cost toward zero for repeated patterns. These figures are self-reported and not yet independently audited."
tags: ["Delta Engine", "Memory Compression", "AI Infrastructure", "RCTDB", "Performance", "Delentia Labs"]
readTime: 8
references:
...
> **Note on the figures below:** the compression, latency, and accuracy numbers in this article are internal benchmarks measured against Delentia Labs' own enterprise (private) workload. They are not yet independently audited, and real-world results vary by input. See the [Delta Engine `compress_context` tool source](https://github.com/delentia-labs/delentia-mcp-ecosystem) for the deployed public implementation's actual, per-request computed behavior.
...
**Result: up to 74% compression vs full-state storage** in internal benchmarks against the RCT Ecosystem enterprise (private) workload (v5.4.5) — not yet independently audited.
...
*All figures below are internal benchmarks (enterprise, private workload), not yet independently audited.*

| Metric | Value |
|---|---|
| Memory compression | **up to 74% lossless** (internal benchmark) |
| Cold start latency | **3–5 seconds** (internal benchmark) |
| Warm recall latency | **<50 milliseconds** (internal benchmark) |
| Semantic similarity threshold | **0.95** (configurable) |
| Storage zones | **3** (Hot/Warm/Cold) |
| RCTDB dimensions | **8** |
| Recall accuracy | **96.1%** (internal benchmark; matches vs expected) |
...
- **Up to 74% compression** (internal benchmark) through delta-only storage (O(n) vs O(n²))
- **<50ms warm recall target** through semantic similarity matching (threshold: 0.95)
