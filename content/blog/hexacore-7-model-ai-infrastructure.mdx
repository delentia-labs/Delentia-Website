...
category: "research"
excerpt: "HexaCore is the multi-model AI routing infrastructure at the heart of the RCT Ecosystem. This article explains how the original 7-model roster (3 Western + 3 Eastern + 1 Regional Thai; since expanded to 9) was selected, balanced, and verified toward a 0.3% hallucination target and an estimated 30-40% cost savings vs single-model deployments — internal benchmarks, not yet independently audited."
tags: ["HexaCore", "Multi-LLM", "AI Infrastructure", "SignedAI", "Constitutional AI", "Delentia Labs"]
readTime: 10
references:
...
The result: a **0.3% hallucination rate target** (industry: 12–15%), an estimated **30–40% cost reduction** vs always using the most capable model, and intended geopolitical balance in every production decision. These are internal benchmark figures, not yet independently audited.

> **Roster update:** the model table below (7 models: 3 Western + 3 Eastern + 1 Thai) reflects the HexaCore configuration at the time this article was written. The roster has since grown to 9 models (adding a local/offline adapter and a low-latency LPU-hosted model) — see `SITE_HEXACORE_COUNT` in the delentia.com codebase for the current count. This article has not yet been updated with the full 9-model table.
...
- [AI Hallucination Prevention — solution page](/solutions/ai-hallucination-prevention) — see how HexaCore's consensus voting achieves the 0.3% hallucination benchmark
- [Dynamic AI Routing](/solutions/dynamic-ai-routing) — the JITNA routing layer that selects which HexaCore model handles each task
- [SignedAI Product](/products/signed-ai) — the verification API built on HexaCore consensus infrastructure
- [RCTDB 8-Dimensional Memory Schema](/blog/rctdb-8-dimensional-memory-schema) — the storage layer that HexaCore models read and write to via Delta Engine
- [FDIA Equation](/protocols/fdia-equation) — the quality gate that validates output quality across all HexaCore tiers
